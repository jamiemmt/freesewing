...
function draftCathrinPanel2({ macro, sa, points, paths, Point, complete, paperless, part }) {
  ...
  points.title = points.LHip // waistCF.shiftFractionTowards(points.underbustGap1Left, 0.5)
  macro('title', {
    nr: 2,
    title: 'Front center lower',
    at: points.title,
  })

  paths.sa2 = paths.side2.offset(sa).attr('class', 'fabric sa').close()
